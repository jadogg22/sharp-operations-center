...
export default function VacationReport({ apiBase, accessPassword, onAccessDenied }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch(`${apiBase}/reports/vacation/preview`, {
          headers: accessPassword ? { 'X-Report-Password': accessPassword } : undefined,
        });
    ...
    void load();
    return () => { active = false; };
  }, [accessPassword, apiBase, onAccessDenied]);
  ...
  return (
    <section className="vacation-card" aria-labelledby="vacation-title">
      ...
      {!loading && !error && <>
        ...
        <div className="vacation-table-wrap"><table className="vacation-table"><thead><tr><th>Employee</th><th>TMS type</th><th>Hours due</th><th>Pay rate</th><th>Estimated amount</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.company_id}-${row.employee_id}`}><td><strong>{row.employee_name}</strong><small>{row.employee_id}</small></td><td><strong>{row.company_id}</strong><small>{row.employee_group}</small></td><td>{row.vacation_hours_due === null ? 'Not entered' : row.vacation_hours_due.toFixed(2)}</td><td>{row.vacation_pay_rate === null ? 'Not entered' : money.format(row.vacation_pay_rate)}</td><td><strong>{money.format(row.amount_due)}</strong></td></tr>)}</tbody></table></div>
      </>}
    </section>
  );
}
